...
use exonum_merkledb::{ObjectHash, Entry, KeySetIndex, ListIndex, MapIndex, MapProof, ProofListIndex, ProofMapIndex, Snapshot, IndexAccess};
...
impl<T: IndexAccess> Schema<T> {
    ...
    pub(crate) fn transaction_results_mut(
        &self,
    ) -> ProofMapIndex<T, Hash, TransactionResult> {
        ProofMapIndex::new(TRANSACTION_RESULTS, self.view)
    }
